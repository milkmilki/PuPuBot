...
- 修复 NapCat 离线或发送失败时，同一个到期定时任务会在每轮 scheduler 中重复调用模型生成回复的问题。
- 定时任务现在会在生成成功后、发送前原子完成或推进下次执行时间；发送失败只记录投递错误，不再重复消耗模型请求。
- 新增发送失败后的双轮轮询回归测试，确保同一任务只触发一次模型生成。

## 2026-06-28

### OneBot 表情消息

- 修复 NapCat actor 收到单独 QQ 表情 / mface 时被解析成空消息、无法进入短期输入 buffer 的问题。
