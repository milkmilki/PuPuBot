...
具体记录规则（非常重要）：
- 任何要写入 summary、event_updates、user_facts、self_facts 的内容，都必须能回答“谁在什么时间/场景做了什么/说了什么/答应了什么”
...
- event_update.title: “2026年5月19日晚上约好一起看《摇曳露营》”
- event_update.summary: “用户在2026年5月19日晚上答应画完图后和仆仆一起看《摇曳露营》，仆仆强调用户跑不掉。”
...
event_updates 用于维护“持续事件线”，目标是直接更新 event_threads/event_steps，减少重复事件：
...
- 每条 event_update 字段：
  ...
  - event_time / time_text / followup_hint / confidence: 同事件线字段
- step_type=time 只能表达推测，summary 必须带“可能/推测/大概/也许”，不要写成确定事实。
"""

_BATCH_REVIEW_BODY = """

task_updates 用来创建、取消或改时间。
...
- thread_key: create 时尽量引用对应事件线；如果同一件事没有事件线，可留空
- title: create 时必填
...
- 其他类型如果时间不明确，通常只更新事件线，不要输出 task_update
