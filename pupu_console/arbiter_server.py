...
def _debounce_idle_seconds() -> float:
    ...
    try:
        return max(0.1, min(60.0, float(raw))) if raw else 30.0
    except ValueError:
        return 30.0


def _debounce_max_seconds() -> float:
    raw = os.environ.get("PUPU_ARBITER_DEBOUNCE_MAX_SEC", "").strip()
    try:
        return max(0.5, min(120.0, float(raw))) if raw else 60.0
    except ValueError:
        return 60.0
...
class _DebounceWatchdog:
    """One asyncio task per group; resettable idle timer with a hard cap.
    ...
    async def _run(self, group_id: str) -> None:
        ...
        try:
            ...
            return
        # If a fresh observe pushed back the cap window we may still be inside
        # the idle window; in that case re-check the cap. If the cap is hit,
        # always fire even if a fresh observe just happened.
...
def build_app(bind_host: str, bind_port: int):
    ...
    def health() -> dict:
        ...
        return {
            ...
            "debounce_max_sec": _debounce_max_seconds(),
        }
