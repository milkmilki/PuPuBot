...
| 组件 | 配置文件 | 作用 |
|------|----------|------|
| bot.py driver | generated `instances/<id>/.env.qq` + instance launcher | `driver="~fastapi"`, HOST/PORT generated from `pupu.yaml` |
...
- 正确示例：`ws://127.0.0.1:8081/onebot/v11/ws`（端口改成你实例目录 `.env.qq` 里 `PORT=` 的值；这个文件由 `pupu.yaml` 的 `napcat.port` 生成）
- 若 NapCat 里 `token` 非空，在 `pupu.yaml` 的 `napcat.access_token` 填同一个值；否则把 NapCat 的 `token` 留空 `""`
...
| 凭证 | `pupu.yaml` | `instance.qq_app_id` + `instance.qq_app_secret` |
