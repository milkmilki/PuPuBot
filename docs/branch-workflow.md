...
仓库提供一个本地 pre-commit hook，用来在提交前检查 staged 文件是否放错分支：
...
- `main` 不允许提交 `desktop/pupu-siri/`、`scripts/show-pupu-siri.ps1` 和 `启动仆仆Siri.bat`。
- `siri` 不允许提交 `pupu/`、`pupu_console/`、`tests/`、`docs/`、`CHANGELOG.md`、`README.md`、`pupu.yaml.example`、`requirements.txt` 等后端/仓库级文件。
- 如果确实在做分支重建或历史修复，可以用 `git commit --no-verify` 临时绕过，但普通开发不应绕过。

Git 没有真正的“编辑前 hook”，所以这个检查发生在提交前。开始改代码前仍然要先看当前分支。
