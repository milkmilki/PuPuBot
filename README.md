...
- **SQLite 事实源 + 内置语义索引**：SQLite 保存结构化事实源，同时维护可删除、可重建的 `semantic_cards` embedding 召回缓存。
- **任务与提醒**：支持 scheduled tasks，并把任务取消、错过、重排等变化写回事件线。

## 记忆系统设计

PuPuBot 的记忆系统分成两层：**SQLite 事实源表**和可选的 **PuPu 内置语义索引**。SQLite 始终是唯一事实源；语义索引不决定 facts 或 events 是否存在，只把 SQLite 中的 summaries、person facts 和 event threads 投影成自然语言 card，再把 card 与 embedding 一起保存到同一个 `pupu.db` 的 `semantic_cards` 表里。你可以把它理解成“结构化主库 + 可重建语义缓存”：主库负责真实、可审计、可迁移，缓存负责让模型更容易想起相关内容。
...
2. 写入 facts 前，`find_related_person_facts()` 会召回本轮相关人物的候选 facts。语义索引可用时优先用 embedding 召回；不可用时退回 SQLite/关键词 overlap。
...
6. 写入 SQLite 后，如果启用了语义索引，再把本轮 `summary` / `person_fact` / `event_thread` 快照投影成可召回 card。语义索引写入失败只影响 RAG 召回，不阻断主库写入。
...
flowchart TD
    ...
    G --> H["写入前候选召回"]
    H --> H1["候选 facts<br/>语义索引召回优先<br/>失败退回 SQLite overlap"]
    ...
    N -- "是" --> P["投影成 source-backed cards<br/>text + source_key + source_version"]
    P --> Q["写入 semantic_cards<br/>float32 embedding BLOB"]
```

### 内置语义索引

SQLite 的事实源表不直接混入 embedding 字段，它们继续用 SQL、FTS5、唯一索引和手写打分保证可控性。embedding 召回层是同一个数据库里的 `semantic_cards` 表：每一行都是由事实源投影出的自然语言 card，带 `source_key`、`source_version` 和 float32 BLOB embedding。

同步到语义索引时，PuPuBot 会把 SQLite 行投影成模型易读的自然语言 card，例如：
...
`source_key` 用来定位 SQLite 源行；`source_version` 用来判断缓存是否过期。因此 `semantic_cards` 里的 card 可以随时删除并从 SQLite 事实源重建。

### 召回流程

聊天时有两种召回路径：

- 未启用语义索引：直接从 SQLite 读取近期 summaries、当前对话人物相关的 `person_facts` 和当前事件线，注入 prompt。
- 启用语义索引：用当前消息作为 query，在 `semantic_cards` 里做本地 cosine 检索，命中后回查 SQLite 源行，再把结果作为“本轮自然想起的事”注入 prompt。

```mermaid
flowchart LR
    A["用户当前消息"] --> B{"语义索引是否启用？"}
    B -- "否" --> C["直接读 SQLite<br/>summaries / person_facts / event_threads"]
    B -- "是" --> D["semantic_cards cosine 检索"]
    D --> E["命中 source_key"]
    E --> F["回查 SQLite 源行"]
    F --> G{"源行仍存在？"}
    G -- "否" --> H["跳过孤儿缓存"]
...
事件线归并本身不依赖语义索引。`find_related_event_threads()` 先用 SQLite FTS5 召回候选，再按关键词 overlap、事件状态、近期活跃度、置信度和人物匹配进行重排。`/events search --debug <query>` 可以看到每条候选的分数组成。

Facts 的写入前候选召回会优先用语义索引里的 `person_fact` card 做 embedding 检索，并回 SQLite 读取最新值；语义索引不可用时退回本地 overlap 搜索。`/facts search --debug <query>` 可以查看候选 fact 的分数、命中词和是否使用语义索引。

### 整理与同步

`/tidy` 现在只负责语义索引一致性，不负责让模型语义归并或删除 SQLite 记忆：

- `/tidy check`：只检查 SQLite 源数据和 `semantic_cards` 是否一致。
- `/tidy` 或 `/tidy apply`：补齐缺失 card，删除孤儿 card、重复 card，刷新版本过期 card。
- `/tidy rebuild`：清空 `semantic_cards`，再按 SQLite 全量重建。
...
推荐使用 Python 3.12 或 3.13。语义索引已经内置到 PuPuBot，不再需要额外安装外部记忆库。
...
- `semantic_index.embed_api_key` / `semantic_index.embed_base_url`：可选。使用百炼 embedding 时填写；视觉工具也会复用这套百炼配置调用 `qwen3.6-flash` 看图。
...
| `/tidy [check|apply|rebuild]` | 检查、同步或重建语义索引 |
| `/score` | 查看好感度 |
| `/history` | 查看最近聊天 |
| `/quit` | 退出 CLI |
...
pupu/memory_index/          Semantic index facade used by agent and commands
